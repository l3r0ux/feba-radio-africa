<template>
  <ion-page class="prayer-page-container">
    ...
    <img
      class="background"
      src="/images/prayer-background.jpg"
      alt="prayer background image"
    />
        ...
        <div class="prayer-container" v-if="!isLoading">
          ...
          <div v-if="afrPrayerError && engPrayerError" class="error-container">
            <h3>
              ...
              {{ afrPrayerError || engPrayerError }}.
            </h3>
            <ion-button class="button" @click="getDailyPrayer()"
              >Retry</ion-button
            >
          </div>
...
const getDailyPrayer = async () => {
  ...
  try {
    const engRes = await axios.get(
      `https://febaradio.co.za/events/event/${engQuery}`
    );
    if (engRes.status === 200) engPrayer.value = scrapePrayerData(engRes.data);
  ...
  try {
    const afrRes = await axios.get(
      `https://febaradio.co.za/events/event/${afrQuery}`
    );
    if (afrRes.status === 200) afrPrayer.value = scrapePrayerData(afrRes.data);
...
const scrapePrayerData = (data: string) => {
  ...
  let prayerStartDate = $(
    ".page-content ul li:first-of-type time:first-of-type"
  ).text();
  let prayerEndDate = $(
    ".page-content ul li:first-of-type time:nth-of-type(2)"
  ).text();

  return {
    prayerText,
    prayerDate: prayerEndDate
      ? `${prayerStartDate} - ${prayerEndDate}`
      : prayerStartDate,
