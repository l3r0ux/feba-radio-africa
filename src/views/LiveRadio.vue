...
const setPlayingRadio = ({
  ...
  if (pausingRadio) playingRadios.value.splice(0, 1);

  if (playingRadios.value.length === 2) {
    const prevAudioEl: HTMLAudioElement | null = document.querySelector(
      `.${playingRadios.value[0]}`
    );
    prevAudioEl?.pause();
...
const radioTiles: Array<RadioTile> = [
  {
    ...
    radioUrl: "https://live.netradio.pro/listen/litalafm/radio.mp3",
  },
  {
